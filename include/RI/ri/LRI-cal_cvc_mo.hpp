...
/// @brief C^mu (m1,m2)[k1,k2] = c^*(m1,s)[k1] C^mu (s,m2)[k2] + C^*mu (s,m1)[k1] c(m2,s)[k2]
...
inline Tensor<Tdata> Cs_ao_mo_to_Cs_mo(
	...
	const std::size_t nocc, const std::size_t nvirt)
{
	...
	for (std::size_t iabf = 0; iabf < nabf; ++iabf)
	{
		for (std::size_t m1 = 0; m1 < nmo1; ++m1)
		{
			...
			{
				Tdata & sum = Cs_mo(iabf, m1, m2);
				for (std::size_t iw = 0; iw < nw; ++iw)
				{
					sum += get_conj(psi_k1(imo1 + m1, iw)) * Cs_ao_mo_k2(iabf, iw, imo2 + m2);
					sum += get_conj(Cs_ao_mo_k1(iabf, iw, imo1 + m1)) * psi_k2(imo2 + m2, iw);
				}
			}
		}
	}
...
#pragma omp for schedule(static) collapse(4)
		for (const Tk k1: k1_list)
		{
			for (const Tk k2: k2_list)
			{
				...
				for (const TA mu : list_I)
				{
					// 2.1 calculate C^\mu (m1,m2)[k2,k1] on-the-fly
...
#pragma omp for schedule(static) collapse(4)
		for (const Tk k1: k1_list)
		{
			for (const Tk k2: k2_list)
			{
				for (const TA mu : list_I)
				{
					// 2.1 calculate C^\mu (m1,m2)[k1,k1] on-the-fly
					const Tensor<Tdata> C_mu_ai = Cs_ao_mo_to_Cs_mo(Cs_ao_mo, map_psi, k1, k1, mu, psi_type[0], psi_type[1], nocc, nvirt);
					...
					for (const TA nu : list_J)
					{
						...
						// 2.2 calculate C^nu (m3,m4)[k1,k2] on-the-fly
						const Tensor<Tdata> C_nu_jb = Cs_ao_mo_to_Cs_mo(Cs_ao_mo, map_psi, k2, k2, nu, psi_type[2], psi_type[3], nocc, nvirt);
...
LRI<TA, Tcell, Ndim, Tdata>::cal_cvc_mo_k(
	const std::map<std::pair<Tk, Tk>, std::map<TA, Tensor<Tdata>>>& Cs_ji,// for B is Cs_bi
	const std::map<std::pair<Tk, Tk>, std::map<TA, Tensor<Tdata>>>& Cs_ab,// for B is Cs_aj
	const std::vector<Tk>& k1_list,
	const std::vector<Tk>& k2_list,
	const std::vector<TA>& list_I,
	const std::vector<TA>& list_J,
...
LRI<TA, Tcell, Ndim, Tdata>::cal_cvc_mo_k_hartree(
	const std::map<std::pair<Tk, Tk>, std::map<TA, Tensor<Tdata>>>& Cs_ai,
	const std::map<std::pair<Tk, Tk>, std::map<TA, Tensor<Tdata>>>& Cs_jb, // for B is Cs_bj
	const std::vector<Tk>& k1_list,
	const std::vector<Tk>& k2_list,
	const std::vector<TA>& list_I,
	const std::vector<TA>& list_J,
